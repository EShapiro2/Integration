...
🔴 **18 of the 32 are held, not routed** (Integration Cowork, 13:52 UTC).  They are refused wholly or partly for `Reader variable "X?" occurs 2 times without ground guard or constant type`, which is the sentence GLP named as a checker fault of their own on 2026-09-20 10:43 UTC, item 8(b): `moded_term.dart:106` counts only a quoted constant as a `String` leaf where `subtyping.dart` counts every unquoted one.  A defect the checker exposes is the owner's to repair; a defect the checker invents is ours, and tasking p99, jurix and federation to rewrite around ours would be the second dressed as the first.  Nothing of that class moves until GLP rules on 8(b).
