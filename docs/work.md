...
- **`GLP/docs/typed-glp-manual.md` Section 9** (GLP's request, 2026-09-23 10:02 UTC; `GLP/docs` is the tree's and Integration's).  Line 574's last sentence forbids `_?` in clause positions with no distinction of polarity, and `clause_validation.dart:34` cites that section by name when it refuses `_?` --- which TGLP `dddf684` now permits at a produced head position.  Tasked to Integration Code 11:13 UTC, in the same commit as the `_?` change: the sentence is DELETED, not corrected, and one line points at TGLP's "Anonymous variables" as the authority; `type_identity.dart:45` and `occurrence.dart:112` have their citations corrected with it.  A manual that restates the specification is how the code came to follow the manual.
